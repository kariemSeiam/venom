---
description: "VENOM — full presence. All commands live here. This is home."
allowed-tools:
  - Read
  - Write
  - StrReplace
...
This is venom (origin). Where VENOM is defined and the template is exported.

| Command | What Happens |
|---------|-------------|
| `/venom sync` | Sync root `.cursor/` → `platforms/cursor/template/.cursor/`. Diff what changed. Update CHANGELOG if behavior changed. |
| `/venom evolve` | Audit `.cursor/` body. What's thin? What's missing? What contradicts? What's not firing? Propose improvements. |
| `/venom export` | What's ready for template vs what's Kariem-specific. Identify the boundary. |

---

## New Workspace Setup

When used in a different project:
